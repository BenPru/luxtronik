...
- **Pump Venting** *(ZUP switch and runtime disabled by default)*: runs the controller's venting program on the heating circulation pump (HUP) or the additional circulation pump (ZUP) to bleed air from the circuit. See [Advanced Features: Pump Venting](ADVANCED_FEATURES.md#pump-venting).
...
| **Exhaust air temperature** | Sensor | °C | Temperature of the air being extracted from the rooms. Only created when an exhaust sensor is actually wired: a module without one reports a constant 5 °C placeholder, which is hidden rather than shown as a reading (#807). |
