...
The diagnostics download redacts your host/IP, credentials, and unique identifiers, and only keeps the MAC address's vendor prefix (see `TO_REDACT` in [diagnostics.py](custom_components/luxtronik2/diagnostics.py)). Your serial number is replaced everywhere by `heatpump_id`, a short hash of it — see [Advanced Features: Serial number and `heatpump_id`](ADVANCED_FEATURES.md#serial-number-and-heatpump_id) for what that does and does not protect. The embedded log records have the same host/IP scrubbed out, but that's a targeted substitution, not full redaction — skim them before attaching if you're concerned about anything else sensitive appearing (this integration doesn't log credentials, but always worth a quick check).
