...
class LuxtronikThermostat(LuxtronikEntity, ClimateEntity, RestoreEntity):
    ...
    def __init__(
        ...
        if configured_indoor_temp_sensor is not None:
            description.luxtronik_key_current_temperature = configured_indoor_temp_sensor
            LOGGER.debug(f'[INIT,{domain}] Using configured indoor temp sensor: {description.luxtronik_key_current_temperature}')
        elif description.luxtronik_key_current_temperature == LuxCalculation.UNSET:
            description.luxtronik_key_current_temperature = LuxCalculation.C0227_ROOM_THERMOSTAT_TEMPERATURE
            LOGGER.debug(f'[INIT,{domain}] Using default indoor temp sensor: {description.luxtronik_key_current_temperature}')
