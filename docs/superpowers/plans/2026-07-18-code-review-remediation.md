...
- **Resolved (2026-07-19)**: chose "revert-on-fail" — the debounced callback in each of `number.py` (`_async_set_native_value`), `climate.py` (`_async_write_temperature`), and `water_heater.py` (`_async_write_temperature`) now catches `HomeAssistantError` around the `coordinator.async_write` call and, on failure, immediately re-syncs the entity via `self._handle_coordinator_update(self.coordinator.data)` (repainting from the last known-good device value) plus an `LOGGER.error(...)` — instead of waiting for the next scheduled poll to silently correct the frontend. Single attempt, no retry.
  - Rejected alternatives (see design discussion): `ir.async_create_issue` repairs are reserved in HA core for persistent/structural problems (deprecated config, reauth, wrong firmware), not per-write transient failures — using one here would spam the Repairs UI. `persistent_notification` is legacy/discouraged for new integration code. A "shared future" restructuring (make `async_set_native_value`/`async_set_temperature` genuinely await the coalesced write's real outcome, so the same `HomeAssistantError` propagates to the original service call) would give byte-for-byte parity with non-debounced writes, but was judged disproportionate: no HA core integration does this, it adds meaningful complexity (custom future-sharing per debounce burst) for a low-severity, low-frequency failure mode (device-side clamping/rejection of a setpoint), and the revert-on-fail fix already closes the actual bug (stale/incorrect value silently lingering) without it.
  - Tests: `tests/test_entity_platforms.py` — `test_async_set_native_value_reverts_on_write_failure` (number), `test_async_write_temperature_reverts_on_write_failure` (climate thermostat + water heater) all simulate `coordinator.async_write` raising `HomeAssistantError` and assert the entity's displayed value reverts to the coordinator's last-known-good data and `async_write_ha_state` fires.
...
- **Resolved (2026-07-19)**: demoted every per-step/per-write chatter `LOGGER.info` to `LOGGER.debug`:
  - `config_flow.py`: all step-progress logging in `async_step_user`/`async_step_manual_entry`/`async_step_dhcp` (discovery start, discovered-device lists, selection-form presentation, DHCP match/already-configured messages) — these fire on every config-flow run/DHCP discovery, not something a normal user needs to see without turning on debug logging. `LOGGER.warning`/`LOGGER.error` calls in the same file were left untouched (already appropriate).
  - `coordinator.py`: the per-coordinator-init "update interval" log and the per-parameter write-confirmation log in `async_write_many` (fires on every single write) — both routine/high-frequency. Left the one-time `LOGGER.info("Library overrides applied...")` and `LOGGER.info("Luxtronik connect to device ... successful!")` at INFO since they're genuine one-time connection-lifecycle events, not steady-state chatter.
  - `__init__.py`: the internal `new_title` debug value and the redundant "integration setup completed" line (HA core's own setup manager already logs setup timing/success). Left `LOGGER.info("Migration to version %s successful")` and the two entity-migration-rename INFO logs alone — these are rare, one-time, actionable events during a version upgrade, not routine operations, so out of scope for this task.
  - No test assertions depended on these specific log levels/messages; verified via full-suite grep before changing.
...
- **M8 Naive timestamp**: `date.py` ~97 uses `datetime.fromtimestamp(value)` (server-local tz, naive). Use explicit tz handling like `sensor.py` ~375 does. **Resolved (2026-07-18)**: changed to `datetime.fromtimestamp(value, UTC)`, matching `sensor.py`'s `format_time`. Added a test asserting the resulting `date()` is anchored to UTC rather than the host's local timezone.
- **M9 Diagnostics redaction**: `diagnostics.py` ~19 redacts username/password (which don't exist in this integration's config) but emits serial number and host unredacted. Redact host, MAC and serial (`TO_REDACT` set), like core integrations. **Resolved (2026-07-18)**: `TO_REDACT` now also covers `CONF_HOST`, `"unique_id"` (serial-derived), `"identifiers"`, `"via_device"`, and `"configuration_url"` (the latter three carry the serial/host inside `coordinator.device_infos`, which is now also run through `async_redact_data`). The MAC keeps its existing bespoke `mac[:9] + "*"` partial mask rather than going through `TO_REDACT` — a deliberate choice, kept on request: the preserved prefix is only the OUI (vendor ID, shared across every device of that chipset, not device-unique), while the masked suffix is the actually device-specific part. This gives some diagnostic value back without exposing anything that identifies the specific device.
- **M10 Discovery port**: `lux_helper.py` ~66 binds UDP 47808 (the standard BACnet port) on all interfaces — conflicts with BACnet tools on the same host. Bind only when actually discovering and close promptly; document the conflict. Also `Luxtronik.__del__` (`lux_helper.py` ~227–229) can perform socket ops during GC on the event-loop thread — guard with try/except and prefer explicit `disconnect()` (already called from coordinator shutdown; verify, then make `__del__` a no-op-on-error). **Resolved (2026-07-18)**: `discover()` already scoped the bind/close to the discovery loop itself (only open for the broadcast+timeout window per port) — added a comment on `LUXTRONIK_DISCOVERY_PORTS` documenting the BACnet conflict since that was the only missing part. `Luxtronik.__del__` now wraps `_disconnect()` in `contextlib.suppress(Exception)`; verified `coordinator.async_shutdown()` (coordinator.py:677) already calls `client.disconnect` explicitly via the executor, so `__del__` is only a best-effort fallback, not the primary cleanup path.
...
- **Resolved (2026-07-19)**: I3 landed first, so the CLAUDE.md wording is now accurate as-is — `async_write`/`async_write_many` compares the post-refresh read-back against the written value and raises `HomeAssistantError` on mismatch, matching "confirms the value stuck" literally. No doc change required.
...
- [x] C1  - [x] C2  - [x] C3
- [ ] I1  - [x] I2  - [x] I3  - [x] I3b  - [x] I4  - [x] I5  - [x] I6  - [x] I7  - [x] I8  - [x] I9  - [x] I10  - [x] I11
- [x] M1  - [x] M2  - [x] M3  - [x] M4  - [x] M5  - [x] M6  - [x] M7  - [x] M8  - [x] M9  - [x] M10  - [x] M11  - [x] M12
