...
async def test_async_step_user_with_no_available_devices(flow_handler):
    flow_handler._async_current_entries = MagicMock(
        return_value=[MagicMock(data={CONF_HOST: "192.168.1.100", CONF_PORT: 8888})]
    )
    flow_handler._discover_devices = AsyncMock(return_value=[("192.168.1.100", 8888)])
...
async def test_async_step_select_devices_success(flow_handler):
    flow_handler._connect_and_get_coordinator = AsyncMock(
        return_value=MagicMock(unique_id="123")
    )
    flow_handler._set_unique_id_or_abort = AsyncMock(return_value=True)

    result = await flow_handler.async_step_select_devices(
        {"selected_devices": ["192.168.1.100:8888"]}
    )
...
async def test_async_step_manual_entry_success(flow_handler):
    ...
    result = await flow_handler.async_step_manual_entry(
        {CONF_HOST: "192.168.1.100", CONF_PORT: 8888}
    )
...
async def test_options_flow_update_options():
    config_entry = MagicMock(
        data={CONF_HOST: "192.168.1.100", CONF_PORT: 8888}, options={}
    )
